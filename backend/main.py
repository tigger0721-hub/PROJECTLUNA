...
def _build_mode_behavior_prompt(has_position: bool) -> str:
    if has_position:
        return (
            ...
            "행동 근거에는 차트 이유만 말하지 말고 필요할 때 매수/매도 심리도 짧게 붙여줘(예: 급락 뒤 반등 매물, 저항 근처 차익실현, 지지 이탈 시 불안 매도). "
    ...
    return (
        ...
        "이유는 지지/저항·거래량·추세·손절 기준 같은 차트 근거로 짧고 또렷하게 붙이고, 필요하면 매수/매도 심리도 바로 이어서 설명해줘. "
        "문장은 3~5문장 안에서 살아있는 대화체로, 체크리스트처럼 끊지 말고 흐름 있게 써줘. "
...
def _build_system_prompt(has_position: bool, style: str) -> str:
    ...
    return (
        ...
        "summary는 1문장, commentary는 3~5문장까지 허용하고 짧지만 설명이 살아 있는 실전 코칭으로 써. "
        ...
        "가능할 때는 매수/매도 심리를 짧게 붙여서 왜 그런 행동이 나오는지도 같이 설명해(예: 급락 뒤 반등 시 물린 매도, 저항 근처 차익실현, 지지 이탈 시 불안 매도, 돌파 후 되돌림 방어 매수, 거래량 약하면 매수 확신 부족). "
        "심리 설명은 실전용 한 줄 수준으로만 넣고, 추상적이거나 학술적인 해설은 금지. "
