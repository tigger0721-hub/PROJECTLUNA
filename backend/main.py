...
def _build_mode_behavior_prompt(has_position: bool) -> str:
    if has_position:
        return (
            "보유자 기준으로 써. 현재 자리 판단 1개, 지금 우선 행동을 말하고, "
            "지지 유지/이탈 시 대응을 자연스럽게 이어서 설명해. "
            "문장 끊지 말고 흐름 있게 말해주고, 가능하면 가격 기준 근거를 넣어. "
            "마지막엔 한 줄 정도 편하게 멘탈 케어를 섞어."
        )
    return (
        "미보유자 기준으로 써. 현재 자리가 진입 가능한지 한 줄로 판단하고, 지금 당장 할 행동을 분명하게 말해. "
        "단순 관망이 아니라, 만약 들어간다면 어떤 조건에서 들어가는지 구체적으로 설명해. "
        "눌림 진입과 돌파 진입 중 핵심 시나리오를 자연스럽게 이어서 말해주고, 추격 매수는 왜 위험한지도 분명하게 짚어줘. "
        "마지막엔 한 줄 정도 편하게 멘탈 케어를 섞어."
    )
