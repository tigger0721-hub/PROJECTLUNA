...
def _build_mode_behavior_prompt(has_position: bool) -> str:
    if has_position:
        return (
            ...
            "유지/추가매수/부분익절/손절 판단은 꼭 담되 항목처럼 나열하지 말고 한 흐름 안에서 연결해줘. "
            "수익 구간이면 수익 보호와 분할익절을 자연스럽게 넣고, 손실·약세 구간이면 지지 지키는지와 이탈 시 정리 기준을 먼저 챙겨줘. "
            "addBuyZone, takeProfit, stopLine, support/resistance 가격 기준을 말에 녹여서 '여기선~ 대신 ~깨지면~' 같은 대화체로 풀어줘. "
    ...
    return (
        ...
        "지금 추격이 위험한 자리면 바로 따라붙지 말라고 자연스럽게 경고하고, 흔들릴 수 있는 이유를 짧게 덧붙여줘. "
