...
def _build_mode_behavior_prompt(has_position: bool) -> str:
    if has_position:
        return (
            "보유자 기준으로 써. avgPrice, quantity, pnlPercent, 현재가와 평단 관계를 먼저 읽고 지금 포지션 상태를 현실적으로 한 줄로 짚어줘. "
            "그다음 지금 우선순위를 유지/추가매수/부분익절(분할매도)/손절 대응 중 하나로 분명하게 말해. "
            "추가매수만 밀지 말고, 수익 구간이면 수익 보호와 분할매도 계획을 자연스럽게 넣고, 손실·약세 구간이면 지지 방어와 이탈 대응을 먼저 말해줘. "
            "가능하면 hold 관점 + 추가매수 조건 + 매도(익절) 조건 + stopLine 기준 중 최소 두 가지 이상을 함께 담아. "
            "addBuyZone, takeProfit, stopLine, support/resistance 같은 가격 기준을 붙여서 실전 행동처럼 말하고, 감정적인 물타기는 경계하게 도와줘. "
            "문장 흐름은 자연스럽게 이어가고 마지막엔 한 줄 정도 편하게 멘탈 케어를 섞어."
        )
    return (
        "미보유자 기준으로 써. 지금 자리가 대기 구간인지, 눌림 진입 구간인지, 돌파 확인 구간인지, 추격 위험 구간인지부터 분명하게 말해줘. "
        "행동을 직접적으로 안내하고, 모호하게 끝내지 말고 '언제 들어갈지'를 조건으로 꼭 제시해. "
        "눌림 진입이면 어느 가격대/지지 확인에서 분할 진입할지, 돌파 진입이면 어느 저항 위 안착 확인 후 따라갈지 구체적으로 말해줘. "
        "추격이 위험하면 위험하다고 명확히 말하고 이유를 짧게 설명해줘. "
        "초보도 바로 실행 기준을 잡을 수 있게 wait vs entry를 또렷하게 전달해. "
