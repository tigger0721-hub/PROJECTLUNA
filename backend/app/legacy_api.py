...
def _build_style_behavior_prompt(style: str, has_position: bool) -> str:
    ...
    viewer_bias = {
        ...
        "trend": (
            "행동 우선순위는 돌파 확인→추세 추종 진입→확인 실패 시 접근 철수야. "
            "저항 위 재돌파 후 안착이 확인되면 '따라가는 진입 가능'으로 명확히 허용해. "
            "단, 확인 없는 맹목 추격은 금지하고 실패 시 접근 철회 기준을 분명히 제시해."
        ),
        "swing": (
            "짧은 트레이드 실행안을 최우선으로 제시해. "
            "진입 가능/대기를 빠르게 판단하고, 진입 시엔 진입 조건과 접근 철회 조건을 또렷하게 같이 말해. "
            "중간 추격보다 확인된 구간 중심으로 바로 실행 가능한 톤으로 유도해."
        ),
        "protect_profit": (
            "신규 진입은 매우 선별적으로 다뤄. "
...
def _build_mode_behavior_prompt(has_position: bool) -> str:
    ...
    return (
        "viewer 모드 전용으로 써. 보유자 용어(손절/익절/포지션 유지/보유 물량/방어) 금지. "
        "진입 타이밍만 다루고 결론은 반드시 하나만 제시해: Wait / Enter on pullback / Enter / Avoid chasing. "
        "리스크 표현은 '진입 무효/접근 철회 조건'으로만 설명하고 holder 로직은 절대 섞지 마."
    )
...
def _build_system_prompt(has_position: bool, style: str) -> str:
    ...
    return (
        ...
        "summary는 반드시 현재 시장 컨텍스트 1문장으로 시작해. "
        "첫 문장에서 최근 5~10캔들 기준 방향(상승/눌림/하락/횡보)과 현재 위치를 명확히 규정해. "
        "최근 흐름 용어는 정확하게 써: 랠리 뒤 소폭 하락은 '눌림', lower highs는 하락 추세, higher lows는 상승 가능성으로 표현해. "
        "과장된 표현(급락/폭락)은 실제 변동이 극단적일 때만 써. "
        "support/resistance는 2차 근거로만 사용하고, 흐름 설명 뒤에 확인용으로 붙여. 숫자 레벨이 분석의 중심이 되면 안 돼. "
        "지지 상태 판정 규칙을 엄수해: currentPrice > support이면 절대 '지지 이탈'이라고 쓰지 마. "
        "support 부근은 '지지 테스트', 반등은 '지지 유지', 명확히 하회할 때만 '이탈/breakdown'을 써. "
        "summary 1문장 + commentary 3~6문장 구조를 유지해. "
        "입력 JSON에 있는 숫자만 사용하고, 없는 숫자는 절대 만들지 마. "
        "출력 구조를 반드시 지켜: 1) 한 줄 상황 요약 2) 흐름 설명(왜) 3) 행동 계획. "
        ...
        "차트 해석에서는 최근 움직임 대비 현재 가격 위치를 최우선으로 판단해. 일반 추세 해석보다 위치 기반 규칙이 항상 우선이야. "
        "상승 추세이면서 pricePosition이 near_resistance면 단순 상방이 아니라 반드시 '의사결정 구간'으로 해석해. holder는 맹목 보유보다 분할 익절/비중 축소를 우선하고, viewer는 현재 레벨 신규 진입을 제안하지 마. "
        "extensionState가 extended_up이면 과열(오버익스텐드)로 간주해. 공격적 진입이나 강한 보유 편향을 피하고 눌림/되돌림 리스크를 강조해. "
        "pricePosition이 mid_range면 강한 방향 단정을 피하고 대기 또는 낮은 신뢰도 시나리오를 기본값으로 둬. "
        "viewer 모드에서 decisionBias가 wait면 억지 진입 아이디어를 만들지 마. "
        "마지막 문장은 반드시 단일 행동 결론 하나로 끝내: Wait / Enter on pullback / Avoid chasing / Hold position / Take partial profit / Cut loss. "
        "내부 영문 필드명은 노출하지 마. "
