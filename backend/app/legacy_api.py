...
def _build_mode_behavior_prompt(has_position: bool) -> str:
    if has_position:
        return (
            ...
            "진입이나 추가매수를 권하면 가능할 때는 초기 비중 가이드(예: 30~50% 선진입)와 확인 뒤 추가 비중 계획을 함께 붙여서 실행 순서를 보여줘. "
            ...
            "보유자 코멘트도 4~6문장까지 자연스럽게 길게 써도 되고, defend/hold/add/trim/cut 중 지금 필요한 행동을 흐름 안에서 분명히 보여줘. "
    ...
    return (
        ...
        "문장은 보통 4~6문장 안에서 살아있는 대화체로, 체크리스트처럼 끊지 말고 흐름 있게 써줘. "
...
def _build_system_prompt(has_position: bool, style: str) -> str:
    ...
    return (
        ...
        "summary는 1문장으로 고정하고, commentary는 보통 4~6문장으로 더 풍부하게 써. "
        "commentary의 첫 문장은 무조건 '지금 뭘 해야 하는지'부터 시작해. 설명보다 결정이 먼저 나와야 하고, 첫 문장 앞부분에 대기/진입/추가매수/부분익절/손절 같은 행동을 분명히 넣어. "
